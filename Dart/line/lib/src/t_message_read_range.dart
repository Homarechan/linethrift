      ...
      switch (field.id) {
        ...
        case RANGES:
          if (field.type == TType.MAP) {
            {
              TMap _map374 = iprot.readMapBegin();
              this.ranges = new Map<String, List<TMessageReadRangeEntry>>();
              for (int _i375 = 0; _i375 < _map374.length; ++_i375) {
                String _key376;
                List<TMessageReadRangeEntry> _val377;
                _key376 = iprot.readString();
                {
                  TList _list378 = iprot.readListBegin();
                  _val377 = new List<TMessageReadRangeEntry>();
                  for (int _i379 = 0; _i379 < _list378.length; ++_i379) {
                    TMessageReadRangeEntry _elem380;
                    _elem380 = new TMessageReadRangeEntry();
                    _elem380.read(iprot);
                    _val377.add(_elem380);
                  }
                  iprot.readListEnd();
                }
                this.ranges[_key376] = _val377;
              }
              iprot.readMapEnd();
            }
          } else {
  ...
  write(TProtocol oprot) {
    ...
    if (this.ranges != null) {
      oprot.writeFieldBegin(_RANGES_FIELD_DESC);
      {
        ...
        for (var elem382 in this.ranges.keys) {
          oprot.writeString(elem382);
          {
            oprot.writeListBegin(new TList(TType.STRUCT, this.ranges[elem382].length));
            for (var elem383 in this.ranges[elem382]) {
              elem383.write(oprot);
            }
            oprot.writeListEnd();
          }
