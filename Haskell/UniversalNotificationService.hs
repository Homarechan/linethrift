...
from_Notify_args record = T.TStruct $ Map.fromList $ M.catMaybes
  [ (\_v10110 -> P.Just (2, ("event",from_GlobalEvent _v10110))) $ notify_args_event record
  ]
...
to_Notify_args (T.TStruct fields) = Notify_args{
  notify_args_event = P.maybe (notify_args_event default_Notify_args) (\(_,_val10112) -> (case _val10112 of {T.TStruct _val10113 -> (to_GlobalEvent (T.TStruct _val10113)); _ -> P.error "wrong type"})) (Map.lookup (2) fields)
  }
...
to_Notify_result (T.TStruct fields) = Notify_result{
  notify_result_e = P.maybe (P.Nothing) (\(_,_val10118) -> P.Just (case _val10118 of {T.TStruct _val10119 -> (to_UniversalNotificationServiceException (T.TStruct _val10119)); _ -> P.error "wrong type"})) (Map.lookup (1) fields)
  }
