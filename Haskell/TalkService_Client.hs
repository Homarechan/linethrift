...
import TalkService
seqid = R.newIORef 0
searchCollection (ip,op) arg_query arg_param = do
  send_searchCollection op arg_query arg_param
  recv_searchCollection ip
send_searchCollection op arg_query arg_param = do
  seq <- seqid
  seqn <- R.readIORef seq
  T.writeMessage op ("searchCollection", T.M_CALL, seqn) $
    write_SearchCollection_args op (SearchCollection_args{searchCollection_args_query=arg_query,searchCollection_args_param=arg_param})
recv_searchCollection ip = do
  T.readMessage ip $ \(fname, mtype, rseqid) -> do
    M.when (mtype == T.M_EXCEPTION) $ do { exn <- T.readAppExn ip ; X.throw exn }
    res <- read_SearchCollection_result ip
    P.maybe (P.return ()) X.throw (searchCollection_result_e res)
    P.return $ searchCollection_result_success res
...
recv_updateProfileAttribute ip = do
  T.readMessage ip $ \(fname, mtype, rseqid) -> do
    ...
    P.return ()
updateProfileAttributes (ip,op) arg_reqSeq arg_request = do
  send_updateProfileAttributes op arg_reqSeq arg_request
  recv_updateProfileAttributes ip
send_updateProfileAttributes op arg_reqSeq arg_request = do
  seq <- seqid
  seqn <- R.readIORef seq
  T.writeMessage op ("updateProfileAttributes", T.M_CALL, seqn) $
    write_UpdateProfileAttributes_args op (UpdateProfileAttributes_args{updateProfileAttributes_args_reqSeq=arg_reqSeq,updateProfileAttributes_args_request=arg_request})
recv_updateProfileAttributes ip = do
  T.readMessage ip $ \(fname, mtype, rseqid) -> do
    M.when (mtype == T.M_EXCEPTION) $ do { exn <- T.readAppExn ip ; X.throw exn }
    res <- read_UpdateProfileAttributes_result ip
    P.return ()
