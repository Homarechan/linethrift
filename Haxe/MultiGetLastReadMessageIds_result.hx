...
class MultiGetLastReadMessageIds_result implements TBase {
  ...
  static var STRUCT_DESC = { new TStruct("MultiGetLastReadMessageIds_result"); };
  static var SUCCESS_FIELD_DESC = { new TField("success", TType.LIST, 0); };
  static var E_FIELD_DESC = { new TField("e", TType.STRUCT, 1); };

  @:isVar
  public var success(get,set) : List< LastReadMessageIds>;
  @:isVar
  ...
  public function get_success() : List< LastReadMessageIds> {
    return this.success;
  }

  public function set_success(success:List< LastReadMessageIds>) : List< LastReadMessageIds> {
        ...
        switch (field.id)
        {
          case SUCCESS_FIELD_ID:
            if (field.type == TType.LIST) {
              {
                var _list923 = iprot.readListBegin();
                this.success = new List< LastReadMessageIds>();
                for( _i924 in 0 ... _list923.size)
                {
                  var _elem925 : LastReadMessageIds;
                  _elem925 = new LastReadMessageIds();
                  _elem925.read(iprot);
                  this.success.add(_elem925);
                }
                iprot.readListEnd();
              }
            } else { 
  ...
  public function toString() : String {
    var ret : String = "MultiGetLastReadMessageIds_result(";
    var first : Bool = true;
