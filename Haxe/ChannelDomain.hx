/**
 * Autogenerated by Thrift Compiler (0.14.0)
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated
 */
package;

import org.apache.thrift.helper.*;
import haxe.io.Bytes;
import haxe.ds.IntMap;
import haxe.ds.StringMap;
import haxe.ds.ObjectMap;

#if flash
import flash.errors.ArgumentError;
#end

import org.apache.thrift.*;
import org.apache.thrift.meta_data.*;
import org.apache.thrift.protocol.*;


class ChannelDomain implements TBase {
  
  static var STRUCT_DESC = { new TStruct("ChannelDomain"); };
  static var HOST_FIELD_DESC = { new TField("host", TType.STRING, 1); };
  static var REMOVED_FIELD_DESC = { new TField("removed", TType.BOOL, 2); };

  @:isVar
  public var host(get,set) : String;
  @:isVar
  public var removed(get,set) : Bool;

  inline static var HOST_FIELD_ID : Int = 1;
  inline static var REMOVED_FIELD_ID : Int = 2;

  private var __isset_removed : Bool = false;

  public function new() {
  }

  public function get_host() : String {
    return this.host;
  }

  public function set_host(host:String) : String {
    this.host = host;
    return this.host;
  }

  public function unsetHost() : Void {
    this.host = null;
  }

  // Returns true if field host is set (has been assigned a value) and false otherwise
  public function isSetHost() : Bool {
    return this.host != null;
  }

  public function get_removed() : Bool {
    return this.removed;
  }

  public function set_removed(removed:Bool) : Bool {
    this.removed = removed;
    this.__isset_removed = true;
    return this.removed;
  }

  public function unsetRemoved() : Void {
    this.__isset_removed = false;
  }

  // Returns true if field removed is set (has been assigned a value) and false otherwise
  public function isSetRemoved() : Bool {
    return this.__isset_removed;
  }

  public function setFieldValue(fieldID : Int, value : Dynamic) : Void {
    switch (fieldID) {
    case HOST_FIELD_ID:
      if (value == null) {
        unsetHost();
      } else {
        this.host = value;
      }

    case REMOVED_FIELD_ID:
      if (value == null) {
        unsetRemoved();
      } else {
        this.removed = value;
      }

    default:
      throw new ArgumentError("Field " + fieldID + " doesn't exist!");
    }
  }

  public function getFieldValue(fieldID : Int) : Dynamic {
    switch (fieldID) {
    case HOST_FIELD_ID:
      return this.host;
    case REMOVED_FIELD_ID:
      return this.removed;
    default:
      throw new ArgumentError("Field " + fieldID + " doesn't exist!");
    }
  }

  // Returns true if field corresponding to fieldID is set (has been assigned a value) and false otherwise
  public function isSet(fieldID : Int) : Bool {
    switch (fieldID) {
    case HOST_FIELD_ID:
      return isSetHost();
    case REMOVED_FIELD_ID:
      return isSetRemoved();
    default:
      throw new ArgumentError("Field " + fieldID + " doesn't exist!");
    }
  }

  public function read( iprot : TProtocol) : Void {
    iprot.IncrementRecursionDepth();
    try
    {
      var field : TField;
      iprot.readStructBegin();
      while (true)
      {
        field = iprot.readFieldBegin();
        if (field.type == TType.STOP) { 
          break;
        }
        switch (field.id)
        {
          case HOST_FIELD_ID:
            if (field.type == TType.STRING) {
              this.host = iprot.readString();
            } else { 
              TProtocolUtil.skip(iprot, field.type);
            }
          case REMOVED_FIELD_ID:
            if (field.type == TType.BOOL) {
              this.removed = iprot.readBool();
              this.__isset_removed = true;
            } else { 
              TProtocolUtil.skip(iprot, field.type);
            }
          default:
            TProtocolUtil.skip(iprot, field.type);
        }
        iprot.readFieldEnd();
      }
      iprot.readStructEnd();

      iprot.DecrementRecursionDepth();
    }
    catch(e:Dynamic)
    {
      iprot.DecrementRecursionDepth();
      throw e;
    }

    // check for required fields of primitive type, which can't be checked in the validate method
    validate();
  }

  public function write(oprot:TProtocol) : Void {
    validate();
    oprot.IncrementRecursionDepth();
    try
    {
      oprot.writeStructBegin(STRUCT_DESC);
      if (this.host != null) {
        oprot.writeFieldBegin(HOST_FIELD_DESC);
        oprot.writeString(this.host);
        oprot.writeFieldEnd();
      }
      oprot.writeFieldBegin(REMOVED_FIELD_DESC);
      oprot.writeBool(this.removed);
      oprot.writeFieldEnd();
      oprot.writeFieldStop();
      oprot.writeStructEnd();
      oprot.DecrementRecursionDepth();
    }
    catch(e:Dynamic)
    {
      oprot.DecrementRecursionDepth();
      throw e;
    }
  }

  public function toString() : String {
    var ret : String = "ChannelDomain(";
    var first : Bool = true;

    ret += "host:";
    if (this.host == null) {
      ret += "null";
    } else {
      ret += this.host;
    }
    first = false;
    if (!first) ret +=  ", ";
    ret += "removed:";
    ret += this.removed;
    first = false;
    ret += ")";
    return ret;
  }

  public function validate() : Void {
    // check for required fields
    // check that fields of type enum have valid values
  }

}

