/**
 * Autogenerated by Thrift Compiler (0.13.0)
 *
 * DO NOT EDIT UNLESS YOU ARE SURE THAT YOU KNOW WHAT YOU ARE DOING
 *  @generated
 */
using System;
using System.Collections;
using System.Collections.Generic;
using System.Text;
using System.IO;
using System.Threading;
using System.Threading.Tasks;
using Thrift;
using Thrift.Collections;

using Thrift.Protocol;
using Thrift.Protocol.Entities;
using Thrift.Protocol.Utilities;
using Thrift.Transport;
using Thrift.Transport.Client;
using Thrift.Transport.Server;
using Thrift.Processor;



public partial class UpdateSquareChatMemberRequest : TBase
{
  private THashSet<SquareChatMemberAttribute> _updatedAttrs;
  private SquareChatMember _chatMember;

  public THashSet<SquareChatMemberAttribute> UpdatedAttrs
  {
    get
    {
      return _updatedAttrs;
    }
    set
    {
      __isset.updatedAttrs = true;
      this._updatedAttrs = value;
    }
  }

  public SquareChatMember ChatMember
  {
    get
    {
      return _chatMember;
    }
    set
    {
      __isset.chatMember = true;
      this._chatMember = value;
    }
  }


  public Isset __isset;
  public struct Isset
  {
    public bool updatedAttrs;
    public bool chatMember;
  }

  public UpdateSquareChatMemberRequest()
  {
  }

  public async Task ReadAsync(TProtocol iprot, CancellationToken cancellationToken)
  {
    iprot.IncrementRecursionDepth();
    try
    {
      TField field;
      await iprot.ReadStructBeginAsync(cancellationToken);
      while (true)
      {
        field = await iprot.ReadFieldBeginAsync(cancellationToken);
        if (field.Type == TType.Stop)
        {
          break;
        }

        switch (field.ID)
        {
          case 2:
            if (field.Type == TType.Set)
            {
              {
                TSet _set479 = await iprot.ReadSetBeginAsync(cancellationToken);
                UpdatedAttrs = new THashSet<SquareChatMemberAttribute>(_set479.Count);
                for(int _i480 = 0; _i480 < _set479.Count; ++_i480)
                {
                  SquareChatMemberAttribute _elem481;
                  _elem481 = (SquareChatMemberAttribute)await iprot.ReadI32Async(cancellationToken);
                  UpdatedAttrs.Add(_elem481);
                }
                await iprot.ReadSetEndAsync(cancellationToken);
              }
            }
            else
            {
              await TProtocolUtil.SkipAsync(iprot, field.Type, cancellationToken);
            }
            break;
          case 3:
            if (field.Type == TType.Struct)
            {
              ChatMember = new SquareChatMember();
              await ChatMember.ReadAsync(iprot, cancellationToken);
            }
            else
            {
              await TProtocolUtil.SkipAsync(iprot, field.Type, cancellationToken);
            }
            break;
          default: 
            await TProtocolUtil.SkipAsync(iprot, field.Type, cancellationToken);
            break;
        }

        await iprot.ReadFieldEndAsync(cancellationToken);
      }

      await iprot.ReadStructEndAsync(cancellationToken);
    }
    finally
    {
      iprot.DecrementRecursionDepth();
    }
  }

  public async Task WriteAsync(TProtocol oprot, CancellationToken cancellationToken)
  {
    oprot.IncrementRecursionDepth();
    try
    {
      var struc = new TStruct("UpdateSquareChatMemberRequest");
      await oprot.WriteStructBeginAsync(struc, cancellationToken);
      var field = new TField();
      if (UpdatedAttrs != null && __isset.updatedAttrs)
      {
        field.Name = "updatedAttrs";
        field.Type = TType.Set;
        field.ID = 2;
        await oprot.WriteFieldBeginAsync(field, cancellationToken);
        {
          await oprot.WriteSetBeginAsync(new TSet(TType.I32, UpdatedAttrs.Count), cancellationToken);
          foreach (SquareChatMemberAttribute _iter482 in UpdatedAttrs)
          {
            await oprot.WriteI32Async((int)_iter482, cancellationToken);
          }
          await oprot.WriteSetEndAsync(cancellationToken);
        }
        await oprot.WriteFieldEndAsync(cancellationToken);
      }
      if (ChatMember != null && __isset.chatMember)
      {
        field.Name = "chatMember";
        field.Type = TType.Struct;
        field.ID = 3;
        await oprot.WriteFieldBeginAsync(field, cancellationToken);
        await ChatMember.WriteAsync(oprot, cancellationToken);
        await oprot.WriteFieldEndAsync(cancellationToken);
      }
      await oprot.WriteFieldStopAsync(cancellationToken);
      await oprot.WriteStructEndAsync(cancellationToken);
    }
    finally
    {
      oprot.DecrementRecursionDepth();
    }
  }

  public override bool Equals(object that)
  {
    var other = that as UpdateSquareChatMemberRequest;
    if (other == null) return false;
    if (ReferenceEquals(this, other)) return true;
    return ((__isset.updatedAttrs == other.__isset.updatedAttrs) && ((!__isset.updatedAttrs) || (TCollections.Equals(UpdatedAttrs, other.UpdatedAttrs))))
      && ((__isset.chatMember == other.__isset.chatMember) && ((!__isset.chatMember) || (System.Object.Equals(ChatMember, other.ChatMember))));
  }

  public override int GetHashCode() {
    int hashcode = 157;
    unchecked {
      if(__isset.updatedAttrs)
        hashcode = (hashcode * 397) + TCollections.GetHashCode(UpdatedAttrs);
      if(__isset.chatMember)
        hashcode = (hashcode * 397) + ChatMember.GetHashCode();
    }
    return hashcode;
  }

  public override string ToString()
  {
    var sb = new StringBuilder("UpdateSquareChatMemberRequest(");
    bool __first = true;
    if (UpdatedAttrs != null && __isset.updatedAttrs)
    {
      if(!__first) { sb.Append(", "); }
      __first = false;
      sb.Append("UpdatedAttrs: ");
      sb.Append(UpdatedAttrs);
    }
    if (ChatMember != null && __isset.chatMember)
    {
      if(!__first) { sb.Append(", "); }
      __first = false;
      sb.Append("ChatMember: ");
      sb.Append(ChatMember== null ? "<null>" : ChatMember.ToString());
    }
    sb.Append(")");
    return sb.ToString();
  }
}

