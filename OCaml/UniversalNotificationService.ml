...
object (self)
  ...
  method grab_event = match _event with None->raise (Field_empty "notify_args.event") | Some _x11019 -> _x11019
  method set_event _x11019 = _event <- Some _x11019
...
let rec read_notify_args (iprot : Protocol.t) =
  let _str11022 = new notify_args in
    ignore(iprot#readStructBegin);
    (try while true do
        let (_,_t11023,_id11024) = iprot#readFieldBegin in
        if _t11023 = Protocol.T_STOP then
          raise Break
        else ();
        (match _id11024 with 
          | 2 -> (if _t11023 = Protocol.T_STRUCT then
              _str11022#set_event (read_globalEvent iprot)
            else
              iprot#skip _t11023)
          | _ -> iprot#skip _t11023);
        iprot#readFieldEnd;
      done; ()
    with Break -> ());
    iprot#readStructEnd;
...
object (self)
  ...
  method get_e = _e
  method grab_e = match _e with None->raise (Field_empty "notify_result.e") | Some _x11025 -> _x11025
  method set_e _x11025 = _e <- Some _x11025
...
let rec read_notify_result (iprot : Protocol.t) =
  let _str11028 = new notify_result in
    ignore(iprot#readStructBegin);
    (try while true do
        let (_,_t11029,_id11030) = iprot#readFieldBegin in
        if _t11029 = Protocol.T_STOP then
          raise Break
        else ();
        (match _id11030 with 
          | 1 -> (if _t11029 = Protocol.T_STRUCT then
              _str11028#set_e (read_universalNotificationServiceException iprot)
            else
              iprot#skip _t11029)
          | _ -> iprot#skip _t11029);
        iprot#readFieldEnd;
      done; ()
    with Break -> ());
    iprot#readStructEnd;
